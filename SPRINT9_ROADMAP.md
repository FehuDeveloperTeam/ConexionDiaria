...
| 9.26 ✅ | Álbum: «Un día como hoy» | El álbum era la única sección que se veía idéntica entre visitas, y con 150 fotos cargadas solo se veían las nueve más nuevas. Trae de vuelta las fotos de este mismo día hace 6 meses y hasta 5 años, consultando por rango de día en vez de filtrar lo ya cargado —los recuerdos que valen son los viejos, justo los que quedan fuera de las 150—. No guarda ningún dato nuevo. |
| 9.27 ✅ | Álbum: reacciones y comentarios | Las fotos no tenían voz: eran archivos, no recuerdos. Corazón de un toque en el visor y un hilo de comentarios por foto, con aviso push a la pareja. Los contadores que el grid usa como señal los escribe una Cloud Function, no el cliente. |
| 9.28 ✅ | Inicio: «Lo próximo» sabe del aniversario | Leía solo la colección `events`, así que sin eventos puestos a mano la tarjeta no se dibujaba — teniendo qué decir, porque el aniversario, los meses cumplidos y los dos cumpleaños siempre se conocen. Ahora los mira todos y, para no repetir la misma fecha dos veces en la pantalla, salta la que el aviso de regalo ya está mostrando. |
| 9.29 ✅ | Tallas: el género se pregunta primero, y lo que declara ella manda | Dos correcciones de la revisión. El asistente armaba la cola de preguntas antes de saber el género, así que a un hombre le preguntaba por sostén y vestido y elegir «Masculino» no lo corregía: la cola ya estaba congelada. Y la precedencia estaba al revés — lo que la persona declara de sí misma pesa más que lo que anotó la pareja, y lo sobreescribe. |
| 9.25 ✅ | Selector de fecha y hora en web | Descubierto al cerrar el sprint, misma familia que el bug del 9.11: `react-native-modal-datetime-picker` no tiene implementación web, así que desde el computador no se podía elegir la fecha del aniversario en Inicio ni la fecha y hora de un evento en Calendario — y no fallaba con un error, simplemente no pasaba nada al tocar. Resuelto con `src/components/DateTimeModal.tsx`, que conserva la forma de props del original para reemplazarlo en el sitio. |
